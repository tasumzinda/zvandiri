...
public class Substance extends Model {
    ...
    public static Substance getItem(String id) {
        return new Select()
                .from(Substance.class).where("id = ?", id).executeSingle();
    }

    public static List<Substance> getAll() {
        return new Select()
                .from(Substance.class)
                .orderBy("name ASC")
                .execute();
    }

    public static void deleteItem(String id) {
        new Delete().from(Substance.class).where("id = ?", id).executeSingle();
    }

    public static void deleteAll() {
        new Delete().from(Substance.class).execute();
    }
    ...
    public static void fetchRemote(Context context, final String userName, final String password){
        String URL = AppUtil.getBaseUrl(context) + "/static/substance";
        StringRequest stringRequest = new StringRequest(Request.Method.GET, URL,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        List<Substance> substanceList = Arrays.asList(AppUtil.createGson().fromJson(response, Substance[].class));
                ...
                new Response.ErrorListener() {
                    ...
                    public void onErrorResponse(VolleyError error) {
                        Log.d("Substance", error.toString());
                    }
                }
        ){
            @Override
