...
public class Referer extends Model implements Serializable{
    ...
    public static Referer getItem(String id) {
        return new Select()
                .from(Referer.class).where("id = ?", id).executeSingle();
    }

    public static List<Referer> getAll() {
        return new Select()
                .from(Referer.class)
                .orderBy("name ASC")
                .execute();
    ...
    public static void deleteItem(String id) {
        new Delete().from(Referer.class).where("id = ?", id).executeSingle();
    }

    public static void deleteAll() {
        new Delete().from(Referer.class).execute();
    }

    public static void fetchRemote(Context context, final String userName, final String password) {
        String URL = AppUtil.getBaseUrl(context) + "/static/refer";
        StringRequest stringRequest = new StringRequest(Request.Method.GET, URL,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        List<Referer> refererList = Arrays.asList(AppUtil.createGson().fromJson(response, Referer[].class));
