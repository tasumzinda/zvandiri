...
public class Position extends Model implements Serializable{
   ...
   // @Expose
    ...
    public static Position getItem(String id) {
        return new Select()
                .from(Position.class).where("id = ?", id).executeSingle();
    }
    ...
    public static void deleteItem(String id) {
        new Delete().from(Position.class).where("id = ?", id).executeSingle();
    }
    ...
    public static void fetchRemote(Context context, final String userName, final String password) {
        String URL = AppUtil.getBaseUrl(context) + "/static/position";
        StringRequest stringRequest = new StringRequest(Request.Method.GET, URL,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        List<Position> positionList = Arrays.asList(AppUtil.createGson().fromJson(response, Position[].class));
